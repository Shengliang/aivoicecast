...
import { ArrowLeft, Sparkles, Wand2, Download, Loader2, RefreshCw, Smartphone, Monitor, LayoutGrid, Check, Info } from 'lucide-react';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [{ text: `Mobile app icon design for "AIVoiceCast". No text. Square format. ${prompt}` }]
        },
        config: {
          imageConfig: {
            aspectRatio: "1:1"
  ...
  return (
    <div className="h-full bg-slate-950 flex flex-col animate-fade-in">
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
        {/* Workspace */}
        <div className="flex-1 overflow-y-auto p-6 md:p-12 flex flex-col items-center justify-center bg-slate-950">
          <div className="relative group">
            {previewMode === 'icon' ? (
              <div className="w-64 h-64 md:w-80 md:h-80 bg-slate-900 rounded-[22%] overflow-hidden shadow-2xl border-4 border-slate-800 relative group">
                {generatedIcon ? (
                ...
                {isGenerating && (
                  <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-sm font-bold text-indigo-200 animate-pulse">Rendering Icon...</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="relative w-72 h-[550px] bg-slate-800 rounded-[3rem] border-[8px] border-slate-900 shadow-2xl overflow-hidden p-6">
                ...
                <div className="grid grid-cols-4 gap-4 mt-8">
                  ...
                  <div className="aspect-square bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-white/10">
                    {generatedIcon && <img src={generatedIcon} className="w-full h-full object-cover" />}
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4 h-16 bg-white/20 backdrop-blur-md rounded-3xl flex items-center justify-around px-2">
                   ...
                   <div className="w-10 h-10 bg-indigo-500 rounded-xl"></div>
          ...
          <div className="mt-8 flex gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-xl font-bold transition-all shadow-xl shadow-indigo-500/20"
            >
              {isGenerating ? <Loader2 size={18} className="animate-spin" /> : <RefreshCw size={18} />}
              {generatedIcon ? 'Regenerate' : 'Generate Identity'}
            </button>
          </div>
        </div>

        {/* Settings Sidebar */}
        <div className="w-full md:w-96 bg-slate-900 border-l border-slate-800 p-6 flex flex-col gap-8 shrink-0">
          <section>
            ...
            <div className="grid grid-cols-1 gap-3">
              {PRESETS.map(preset => (
                <button
                  ...
                  className={`text-left p-4 rounded-xl border transition-all ${activePreset.id === preset.id && !customPrompt ? 'bg-indigo-900/30 border-indigo-500' : 'bg-slate-800 border-slate-700 hover:border-slate-500'}`}
                >
                  <p className="font-bold text-sm text-white">{preset.name}</p>
                  <p className="text-[10px] text-slate-500 line-clamp-1 mt-1">{preset.prompt}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="flex-1 flex flex-col">
            ...
            <textarea
              ...
              placeholder="Describe your vision (e.g. 'Golden ratios, matte black, neon waves')..."
              className="w-full flex-1 bg-slate-950 border border-slate-700 rounded-xl p-4 text-sm text-slate-300 focus:ring-2 focus:ring-indigo-500 outline-none resize-none"
            />
          </section>
          ...
          <div className="bg-indigo-900/20 border border-indigo-500/30 p-4 rounded-xl flex gap-3">
            ...
            <p className="text-[10px] text-indigo-200 leading-relaxed">
              These icons are generated specifically for high-density smartphone screens. 
              The AI will automatically center the composition and avoid text for best legibility.
            </p>
          </div>
        </div>
      </div>
